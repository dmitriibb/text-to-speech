...
OpenVoice backend work is now wired to a real runtime. The repo contains a new `apps/open_voice_be` FastAPI app with async speech-job endpoints, disk-backed `.json` job metadata, `.wav` reference and result storage under `storage/`, deterministic model downloads under `models/`, and a desktop Voice Lab OpenVoice section for backend URL entry, health checks, WAV selection, and async job polling. The backend uses official OpenVoice V2 checkpoints plus official MeloTTS English-v2 in CPU mode as the first Windows-target runtime. The desktop client polls job status with 1s, 2s, 3s, and so on up to 10s. The backend now also serves a lightweight browser admin at `/admin` that lists the available backend models, shows which ones are downloaded when applicable, lets the user choose the current backend model, and shows saved backend jobs with their referenced files. Deleting a backend job from that admin removes its job record and job-owned files.

The backend now also has an OmniVoice runtime path behind the same `GET /health`, `POST /jobs`, `GET /jobs/{job_id}`, and `GET /jobs/{job_id}/result` contract. Model selection can now switch between the original OpenVoice base-speaker options and an `omnivoice-multilingual` backend model. This keeps the desktop client contract stable while allowing a heavier multilingual cloning runtime to live behind the same desktop-only backend boundary.

That shared-environment approach hit a real dependency conflict: OpenVoice's MeloTTS path requires an old pinned `transformers`, while OmniVoice requires a much newer one. To keep local startup practical, the repo now also contains a separate `apps/omnivoice_be` backend app with its own Python environment and the same desktop-facing job API. The intended near-term path is: keep `apps/open_voice_be` for OpenVoice experiments and use `apps/omnivoice_be` for OmniVoice runs.

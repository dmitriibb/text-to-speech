...
8. The shared player pauses and resumes the currently loaded generated audio without resetting progress.
9. Before cancelling a task or removing generated audio from the task list, the app pauses active playback and asks for confirmation.
10. Desktop can export a copy through the task-row save action and Android can share the `.wav` through the system share sheet.
11. When the user confirms dismissing a generated-audio task, the app removes the task metadata and deletes that generated local `.wav`.

...
8. Multilingual or family-specific runtime assets such as extra lexicons, rule FST or FAR files, dictionaries, bundled speaker files, Supertonic runtime files, or selectable generation-language lists must also be declared in the catalog instead of being inferred in app code.
...
- Multilingual runtime assets such as Kokoro Chinese lexicons, rule FSTs, dictionaries, or Supertonic model component files must be declared in the catalog and passed through shared runtime metadata.
- Model-specific generation options such as Supertonic's default target language, selectable target-language list, and generation step count must come from catalog metadata, not app hard-coding.
